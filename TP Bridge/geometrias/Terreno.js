function Terreno(anchoCosta,largoCosta,anchoRio,anchoCalle,nroTorres,sepTensor,
  ...
  Terreno.prototype.agregarArboles = function() {
    for(var i = 0;i<10;++i){
      ...
      //PRIMERA COSTA
      if (randomNumber < 0.5){
        x = Utils.getRandomBetweenMaxMin(1,this.xCostaDer-3);
        z = -Utils.getRandomBetweenMaxMin(1,this.largoCosta-1);
      }
      //SEGUNDA COSTA
      else{
        x = Utils.getRandomBetweenMaxMin(this.xCostaDer+this.anchoRio+1,this.xCostaDer+this.anchoRio+this.xCostaDer-1);
        z = -Utils.getRandomBetweenMaxMin(1,this.largoCosta-1);
      }
      arbol.init(x,0,z);
